...
public class ColorPickerDialog extends Dialog implements
    ...
    public ColorPickerDialog(Context context, int theme, int initialColor,
            int androidColor, int zeusColor) {
        super(context, theme);
        mInitialColor = initialColor;
        mAndroidColor = androidColor;
        mZeusColor = zeusColor;
        if (mAndroidColor != 0x00000000 && mZeusColor != 0x00000000) {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.palette_zeus) {
            mPalette = PALETTE_ZEUS;
            mColorTransitionAnimator.start();
            return true;
            ...
            return true;
        } else if (item.getItemId() == R.id.reset_zeus) {
            mColorPicker.setColor(mZeusColor, true);
            return true;
        }
        return false;
    }
